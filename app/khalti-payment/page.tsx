...
import styles from './khalti-payment.module.css';
...
useEffect(() => {
  const fetchDummyData = async () => {
    try {
      const response = await fetch(`/api/dummy-data?method=khalti${queryAmount ? `&amount=${queryAmount}` : ""}`);
      if (!response.ok) throw new Error("Failed to fetch dummy data");
      const data = await response.json();
      // Always set productName and transactionId from API
      setProductName(data.productName);
      setTransactionId(data.transactionId);
      // Only set amount if not present in query
      if (!queryAmount) setAmount(data.amount);
    } catch (error) {
...
{/*
useEffect(() => {
  if (queryAmount && queryAmount !== amount) {
 ...
 useEffect(() => {
  if (!queryAmount && !amount) {
    setAmount(queryAmount);
  } else if (!queryAmount && !amount) {
    // Only fetch dummy data if amount is not present in the URL or state
    const fetchDummyData = async () => {
      try {
  ...
  return (
    <>
      <Script
        ...
        strategy="lazyOnload"
      />
      <div className={styles.khaltiRoot}>
