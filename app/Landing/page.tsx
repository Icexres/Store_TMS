...
const Landing = () => {
  ...
  return ( <>
      {!showLanding &&(
        <video key={showLanding ? 'ended':"playing"} 
        src="/intro.mp4" autoPlay className='fixed top-0 left-0 w-full h-full object-cover z-50'
        onEnded={() => setShowLanding(true)} />
      )}  
    {showLanding && (
      <audio ref={audioRef} src="/menumusic.mp3" loop />
    )}
      {showLanding && (
        <div className='landiing-container'>
          <div className='header bg-[#Eff2c0] text-black p-1'>
            <div className='logo text-4xl font-extrabold'>BMS</div>
          </div>
          {/* end of header */}
          <div className='content text-center mt-40 flex flex-col gap-8'>
            <div className='text-4xl font-serif font-extrabold'>Get Started With BMS</div>
